...
def main(infile, outpath_mpf, outpath_reference, number_questions, number_nan):
    ...
    question_id_column = 'question_id_drh'
    minimize_value = 0
    ...
    ## rename for clarity 
    d = d.rename(columns = {'related_q_id': 'question_id_drh',
                            'related_q': 'question_drh'})
    
    ## replace with unknown (currently not used - treated as NAN)
    d.replace({'answers': {"Field doesn't know": 'Unknown', 
                           "I don't know": 'Unknown'}},
              inplace = True)

    ## reference documents (used for saving reference documents)
    entry_reference = d[['entry_id', 'entry_name']].drop_duplicates()
    question_reference = d[['question_id_drh', 'question_drh']].drop_duplicates()
    ...
    ## recode answer values
    d[answer_column] = d[answer_column].map(answer_coding).fillna(default).values.astype(int)

    # remove questions with non-binary answers
    d = cf.remove_non_binary(d, answer_column, question_id_column, default)

    # create grid of entry_id/question_id_drh
    # create entry_id/question_id_drh weighted by answers for combinations with (Yes/No) answers
    ...
    best_questions = cf.subset_nbest(question_entry_grid, number_questions,
                                     minimize_value, question_id_column, nan_column) 
    ...
    # for each entry_id/question_id_drh create tuples of combinations and add to list
    # i.e. [(question_id_drh, entry_id, answer, weight)]
    # if there are more than one answer to entry_id/question_id_drh then we expand and weight
    ...
    if weighted_combinations_list: 
        ...
        data_csv = cf.to_dataframe(weighted_combinations_list)
        ...
        # save data 
        cf.save_output(best_entries, entry_reference, question_reference, 
